...
from scriptsLibrary.functions import login, signup, is_superuser, show_all_books, search_books, get_book_by_id, \
    is_book_registered, unregister_book, register_book, get_registered_books, search_registered_books, delete_book, \
    add_book, edit_book, show_users, search_users, get_user_by_id, get_user_books_info
from scriptsLibrary.models import user_exists
...
class LogIn:
    ...
    def login(self):
        ...
        if is_registered != -1:
            self.window.destroy()
            Home(is_superuser(self.superuser), self.user_id)
        else:
...
class RegisteredBooks:
    def __init__(self, user_id, superuser):
        ...
        self.all_books = get_registered_books(self.user_id)[0]
    ...
    def search_button_command(self):
        keyword = self.search_entry.get()
        if keyword:
            data = search_registered_books(keyword, self.all_books[1])
            self.listbox.delete(0, END)
            for item in data:
                self.listbox.insert(END, f'{item[0]}. {item[1]}')
    ...
    def on_double_click(self, event):
        book_id = int(str(self.listbox.get(self.listbox.curselection())).split('.')[0])
        self.window.destroy()
        SpecificBook(user_id=self.user_id, superuser=self.superuser, book_id=book_id, current_window="RegisteredBooks")
...
class SpecificBook:
    def __init__(self, user_id, superuser, book_id, current_window):
        ...
        book_to_str = f'''
        Name: {self.book_info['name']}
        Author: {self.book_info['author']}
        Category: {self.book_info['category']}
        Description: {self.book_info['description']}
        Unit: {self.book_info['unit']}
        '''
...
class EditBook:
    ...
    def edit_button_command(self):
        ...
        self.window.destroy()
        SpecificBook(superuser=self.superuser, user_id=self.user_id, book_id=self.book_id, current_window="EditBook")
...
class SpecificUser:
    def __init__(self, user_id, superuser, current_user):
        ...
        book_to_str = f'''
        Id: {self.client_info['id']}
        Name: {self.client_info['name']}
        Surname: {self.client_info['surname']}
        Username: {self.client_info['username']}
        Gmail: {self.client_info['gmail']}
        Book:'''

        self.client_book_info = get_user_books_info(self.client_info['books'])
        length_books_data = len(self.client_book_info)
        # print(self.client_book_info)
        if length_books_data == 0:
            book_to_str += " no books"
        else:
            count = 1
            while count < length_books_data:
                if count % 2 == 0:
                    book_to_str += f"\n{self.client_book_info[count - 1][1]}, "
                else:
                    book_to_str += f"{self.client_book_info[count - 1][1]}, "
                count += 1
